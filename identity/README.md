...
is that layer, a sibling of `app/` (Pie Timers) and `supabase/`
...
- **The layer does not decide what a session unlocks beyond capabilities.** A
  product asks `hasCapability('pie-timers', 'can_sync')` and never asks
...
- **The layer does not request product-specific OAuth scope.** Pie Timers' Google
...
- **The layer does not give two subdomains a shared signed-in session for free.**
  ...
  worth a short conversation of its own before the second app arrives.

## Deploying this folder

`identity/` is checked out at the repo root. The Pages workflow
(`.github/workflows/deploy.yml`) copies the folder into the published site
